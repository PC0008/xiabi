...
| 命令 | 状态 | 耗时 | 说明 |
| --- | --- | --- | --- |
| typecheck | 通过 | 3.4s | 服务端与静态前端类型/源码检查 |
...
| check:generation-task-safety | 通过 | 0.5s | 写信任务轮询与重试幂等安全门 |
...
| verify:order-payment-switch | 通过 | 0.5s | 支付开关和续付边界 |
| verify:live | 通过 | 30.3s | 线上入口/API 边界/截图巡检 |
| verify:journey | 通过 | 37.2s | 移动端用户主流程旅程 |
| verify:production | 通过 | 4.9s | 生产基础验收，不触发外部付费调用 |
| acceptance:inputs | 通过 | 0.4s | 最终人工验收输入准备度清单，不触发外部付费调用 |
| delivery:status | 通过 | 0.5s | 最终交付状态清单生成 |
