...
| 命令 | 状态 | 耗时 | 说明 |
| --- | --- | --- | --- |
| typecheck | 通过 | 5.4s | 服务端与静态前端类型/源码检查 |
...
| check:sms-code-safety | 通过 | 0.6s | 短信验证码哈希安全边界 |
...
| verify:live | 通过 | 38.6s | 线上入口/API 边界/截图巡检 |
| verify:journey | 通过 | 43.7s | 移动端用户主流程旅程 |
| verify:production | 通过 | 6.1s | 生产基础验收，不触发外部付费调用 |
