...
| 命令 | 状态 | 耗时 | 说明 |
| --- | --- | --- | --- |
| typecheck | 通过 | 5.1s | 服务端与静态前端类型/源码检查 |
| build | 通过 | 1.2s | 静态 Web 构建 |
| check:ui | 通过 | 0.8s | 用户端/后台关键交互覆盖标记 |
| check:env-contract | 通过 | 0.7s | 服务端环境变量与部署样例契约 |
| check:admin-config-control | 通过 | 0.7s | 后台配置真实控制用户端和服务端 |
| check:sensitive-output-safety | 通过 | 0.7s | 供应商失败和敏感输出安全边界 |
| check:admin-permissions | 通过 | 0.7s | 后台高风险权限边界 |
| check:public-session-safety | 通过 | 0.7s | 公开写入与外部调用接口会话边界 |
...
| verify:live | 通过 | 35.8s | 线上入口/API 边界/截图巡检 |
| verify:journey | 通过 | 43.3s | 移动端用户主流程旅程 |
| verify:production | 通过 | 5.5s | 生产基础验收，不触发外部付费调用 |
