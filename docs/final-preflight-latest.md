...
| check:public-session-safety | 通过 | 0.5s | 公开写入与外部调用接口会话边界 |
...
| verify:live | 通过 | 33.4s | 线上入口/API 边界/截图巡检 |
| verify:journey | 通过 | 37.0s | 移动端用户主流程旅程 |
| verify:production | 通过 | 4.7s | 生产基础验收，不触发外部付费调用 |
