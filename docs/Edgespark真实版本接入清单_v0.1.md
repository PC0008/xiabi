...
```powershell
edgespark var set PUBLIC_BASE_URL=https://你的域名 PAYMENT_PROVIDER=wechat PAYMENT_NOTIFY_URL=https://你的域名/api/webhooks/wechat-pay WECHAT_PAY_APP_ID=你的appid WECHAT_PAY_MCH_ID=你的商户号 SMS_PROVIDER=你的短信服务商 VOICE_PROVIDER=你的语音服务商 TASK_QUEUE_NAME=xiabi-generation ADMIN_INITIAL_USERNAME=admin
```

密钥：

```powershell
edgespark secret set ADMIN_INITIAL_PASSWORD ADMIN_PASSWORD_PEPPER WECHAT_PAY_API_V3_KEY WECHAT_PAY_PRIVATE_KEY WECHAT_PAY_CERT_SERIAL_NO SMS_API_KEY SMS_API_SECRET VOICE_API_KEY VOICE_API_SECRET
```
