...
```powershell
edgespark var set PUBLIC_BASE_URL=https://你的域名 PAYMENT_PROVIDER=wechat PAYMENT_NOTIFY_URL=https://你的域名/api/webhooks/wechat-pay WECHAT_PAY_APP_ID=你的appid WECHAT_PAY_MCH_ID=你的商户号 SMS_PROVIDER=你的短信服务商 VOICE_PROVIDER=minimax TASK_QUEUE_NAME=xiabi-generation ADMIN_INITIAL_USERNAME=admin
```
