...
> 历史说明：本文是从 mock 支付切换到正式支付前的设计稿。当前微信 H5/JSAPI 下单、回调验签/解密、查单补偿、订单权益发放和后台补偿按钮已经接入；最新实现以 `server/src/routes/orders.ts`、`server/src/routes/webhooks.ts`、`server/src/adapters/payment/wechat.ts`、`server/src/domain/entitlements.ts` 和生产验收报告为准。当前外部阻塞是微信商户产品权限未开通。
