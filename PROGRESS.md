...
- 最终人工验收清单口径补齐：第一批“后台账号”验收现在明确包含短信供应商无发码自检和微信支付无订单自检，避免最终交付时漏跑新加入的签名/模板、商户签名、平台证书前置检查。
- 微信支付自检继续收紧：无订单自检现在会先用商户私钥做本地签名探针，手动配置平台公钥时也会导入校验公钥/证书格式；后台实时结果新增“商户签名”状态，避免私钥 PEM、证书序列号或平台公钥粘贴错误拖到真实下单/回调时才暴露。
- 微信支付最终验收前置补强：新增后台“微信支付自检”接口和按钮，不创建订单、不拉起付款，先检查商户配置、商户签名、APIv3 Key、平台证书手动配置或自动拉取能力以及公众号授权配置；`verify:production` 新增 `wechat provider config` 证据项，便于正式下单前先排除证书/验签类问题。
...
- 支付结果刷新风控补强：用户端刷新待支付订单结果前，会按会话检查 `order.payment_status_check` 审计事件，同一会话每小时最多 60 次微信查单刷新，超限返回 `payment_status_rate_limited`；刷新查单和刷新确认已支付都会进入后台审计，避免回调延迟时反复点击刷新放大微信查单成本。
- 支付拉起风控补强：用户创建订单或继续支付前，会按会话检查 `order.payment_attempt` 审计事件，同一会话每小时最多 20 次支付拉起，超限返回 `payment_rate_limited`，避免微信支付真实接入后被重复点击或异常重试放大外部调用成本；用户端同步展示友好提示并保留订单/信件。
- 支付拉起审计链路补强：用户创建订单或继续支付时，服务端会记录 `order.payment_attempt`、`order.payment` 和 `order.payment_failed` 审计事件，包含订单号、商户订单号、商品类型、金额、H5/JSAPI 模式和失败分类；微信产品权限阻塞等失败响应也会带回订单 ID，生产验收在有后台账号时会核对对应订单审计链路。
- 生产短信验收补强：`verify:production` 在真实发送短信或绑定验证码时，若同时提供后台账号，会继续查询后台审计日志，确认对应脱敏手机号出现 `sms.send_attempt` 以及成功/失败事件，并检查审计详情没有泄露完整手机号或验证码字段；最终人工验证批次同步要求短信验收携带后台账号。
- 短信审计可观测性补强：短信验证码发送现在会写入 `sms.send_attempt`、`sms.send` 和 `sms.send_failed` 审计事件，只记录脱敏手机号、供应商、BizId 或错误分类，不记录验证码明文；后台日志审计和生产验收同步支持按短信事件筛选。
- 后台语音审计筛选补强：管理后台“日志审计”页新增语音播放/语音输入尝试与成功事件筛选，并把 `voice` 对象纳入对象筛选；生产验收在提供后台账号时会覆盖语音审计筛选接口。
...
- 后台审计可用性补强：`/api/public/admin/audit-logs` 支持按事件、对象和操作人类型筛选并校验筛选长度；管理后台“日志审计”页新增事件/对象筛选控件，生产验收在有后台账号时会覆盖筛选接口，方便运营排查配置、账号、订单、回调和反馈变更。

- 线上边界验收补强：`verify:live` 新增短信手机号格式、语音空文本、语音文本过长和不支持音频 MIME 的负向用例，覆盖这些接口在真实公网环境下不会触发外部供应商调用且能返回明确错误码。
...
- 导出目前不是二进制 PDF 文件，而是服务端可打印 HTML、TXT 文本版和 DOCX 文档版；用户可以在浏览器内把打印版保存为 PDF，如需服务端直接生成 PDF 还要继续接 PDF 渲染能力。
