...
- 最终无外部费用预检：`npm run verify:preflight` 会串行运行类型检查、构建、静态回归、环境变量契约、支付权益安全门、线上巡检、移动端旅程、生产基础验收和交付状态生成，并写入 `docs/final-preflight-latest.md`、`docs/production-readiness-preflight-latest.md` 和 `docs/delivery-status-preflight-latest.md`；它不会主动触发 DeepSeek、短信、微信支付、MiniMax TTS 或 ASR 的真实调用。
